...
def index(request):
    ...
    for post in posts:
        ...
        try:
            ...
            page = p.page(1)
    return render(request, "index.html", {"activePosts": activePosts, "page": page, "isLiked": isLiked, "likeIds": likeIds,
                                           "postform": postform
                                           })
...
def newmaterial(request):
    if request.method == "POST":
        material = Material(owner=request.user, title=request.POST["title"], author=request.POST["author"], isCourse=request.POST["isCourse"], startDate=request.POST["startDate"])
        material.save()
        return HttpResponseRedirect(reverse("profile", kwargs={'username':request.user.username}))
...
def profile(request, username):
    ...
    materials = Material.objects.filter(owner=user.id).order_by('-startDate')
    activePosts = {}
    isLiked = {}
    likeIds = {}
    followIds = {}
...
def following(request):
    ...
    try:
        ...
        page = p.page(1)
    return render(request, "following.html", {"activePosts": activePosts, "page": page, "isLiked": isLiked, "likeIds": likeIds})
